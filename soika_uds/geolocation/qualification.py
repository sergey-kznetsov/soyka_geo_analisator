...
class FrozenDict(dict):
    ...
    def _blocked(*args: object, **kwargs: object) -> None:
        raise TypeError("qualified evidence is immutable")
...
class GateResult:
    ...
    def __post_init__(self) -> None:
        ...
        object.__setattr__(self, "evidence", _string_tuple(self.evidence, "gate.evidence"))
...
class GeolocationModelAudit:
    ...
    def __post_init__(self) -> None:
        for name in ("component_id", "component_version", "artifact_kind", "license_id"):
            object.__setattr__(self, name, _required(getattr(self, name), f"audit.{name}"))
        object.__setattr__(
            self,
            "artifact_sha256",
        ...
        for name in (
            ...
            object.__setattr__(self, name, _boolean(getattr(self, name), f"audit.{name}"))
        object.__setattr__(
            self,
            "evidence_urls",
    ...
    def from_dict(cls, payload: Mapping[str, Any]) -> GeolocationModelAudit:
        ...
        unknown = set(data) - required - {"limitations"}
        missing = required - set(data)
        if unknown or missing:
            raise ValueError(f"invalid audit fields: unknown={sorted(unknown)}, missing={sorted(missing)}")
        return cls(
...
class GeolocationValidationCase:
    ...
    def __post_init__(self) -> None:
        for name in ("message_key", "city", "model_text", "expected_mention"):
            object.__setattr__(self, name, _required(getattr(self, name), f"case.{name}"))
        if not isinstance(self.expected_point, GeoPoint):
            raise TypeError("case.expected_point must be GeoPoint")
        if not isinstance(self.expected_kind, LocationKind):
            object.__setattr__(self, "expected_kind", LocationKind(self.expected_kind))
        tolerance = _non_negative(self.tolerance_m, "case.tolerance_m")
        if tolerance <= 0:
            raise ValueError("case.tolerance_m must be positive")
        object.__setattr__(self, "tolerance_m", tolerance)
        evidence = _string_tuple(self.evidence_urls, "case.evidence_urls")
        if len(evidence) < 2:
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            ...
            "expected_point": _point_dict(self.expected_point),
    ...
    def from_dict(cls, payload: Mapping[str, Any]) -> GeolocationValidationCase:
        ...
        point = _mapping(data.get("expected_point"), "case.expected_point")
        return cls(
            message_key=data.get("message_key"),
            city=data.get("city"),
            model_text=data.get("model_text"),
            expected_mention=data.get("expected_mention"),
            expected_point=GeoPoint(
                longitude=point.get("longitude"),
                latitude=point.get("latitude"),
            ),
            expected_kind=LocationKind(data.get("expected_kind")),
            tolerance_m=data.get("tolerance_m"),
            evidence_urls=tuple(data.get("evidence_urls", ())),
        )
...
class GeolocationValidationManifest:
    ...
    def __post_init__(self) -> None:
        for name in ("dataset_id", "version", "license_id", "attribution", "approval_reference"):
            object.__setattr__(self, name, _required(getattr(self, name), f"validation.{name}"))
        levels = tuple(
        ...
        if self.declared_digest is not None:
            declared = _sha256(self.declared_digest, "validation.declared_digest")
    ...
    @property
    def counts_by_city(self) -> FrozenDict:
        counts = Counter(item.city for item in self.cases)
        return FrozenDict(dict(sorted(counts.items())))
    ...
    def from_dict(cls, payload: Mapping[str, Any]) -> GeolocationValidationManifest:
        ...
        return cls(
            ...
            approved_levels=tuple(LocationKind(item) for item in data.get("approved_levels", ())),
            cases=tuple(
...
class GeolocationThresholds:
    ...
    def __post_init__(self) -> None:
        object.__setattr__(self, "min_samples", _positive_int(self.min_samples, "thresholds.min_samples"))
        object.__setattr__(
            self,
            "min_samples_per_city",
            _positive_int(self.min_samples_per_city, "thresholds.min_samples_per_city"),
        )
        for name in (
            ...
            object.__setattr__(self, name, _probability(getattr(self, name), f"thresholds.{name}"))
        for name in ("max_median_distance_m", "max_p95_distance_m"):
            object.__setattr__(self, name, _non_negative(getattr(self, name), f"thresholds.{name}"))
...
class GeolocationQualificationReport:
    ...
    def __post_init__(self) -> None:
        object.__setattr__(
            ...
            _boolean(self.approved_for_production, "report.approved_for_production"),
        )
        object.__setattr__(self, "validation_digest", _sha256(self.validation_digest, "report.validation_digest"))
        object.__setattr__(self, "validation_version", _required(self.validation_version, "report.validation_version"))
        object.__setattr__(
            self,
            "approved_levels",
            tuple(item if isinstance(item, LocationKind) else LocationKind(item) for item in self.approved_levels),
        )
        object.__setattr__(self, "metrics", _freeze(_mapping(self.metrics, "report.metrics")))
        object.__setattr__(
            self,
            "provider_policy",
            _freeze(_mapping(self.provider_policy, "report.provider_policy")),
        )
        object.__setattr__(self, "gates", tuple(self.gates))
        object.__setattr__(self, "report_digest", _sha256(self.report_digest, "report.report_digest"))
        ...
        if self.approved_for_production != all(gate.state is GateState.PASSED for gate in self.gates):
    ...
    def payload_without_digest(self) -> dict[str, Any]:
        return {
            ...
            "provider_policy": _plain(self.provider_policy),
        }
...
def _gate(code: str, passed: bool, detail: str, evidence: Sequence[str] = ()) -> GateResult:
    return GateResult(
        code=code,
...
def qualify_geolocation(
    *,
    ...
    resolved = thresholds or GeolocationThresholds()
    policy = dict(
        provider_policy
        or {
    ...
    metric_copy = _plain(_freeze(_mapping(metrics, "metrics")))
    samples = int(metric_copy.get("samples", 0))
    extraction = float(metric_copy.get("extraction_exact_rate", 0.0))
    low_confidence = float(metric_copy.get("low_confidence_rate", 1.0))
    resolution = float(metric_copy.get("resolution_rate", 0.0))
    within = float(metric_copy.get("within_tolerance_rate", 0.0))
    kind = float(metric_copy.get("kind_accuracy", 0.0))
    median = metric_copy.get("median_distance_m")
    p95 = metric_copy.get("p95_distance_m")
    cities = _mapping(metric_copy.get("cities", {}), "metrics.cities")
    gates = (
        _gate(
            "MODEL_AUDIT",
        ...
        _gate(
            "MODEL_SMOKE",
            metric_copy.get("model_smoke_passed") is True,
            "the pinned package imports and produces a location mention",
        ),
        _gate(
            "VALIDATION_MANIFEST",
        ...
        _gate(
            "SAMPLE_SIZE",
            samples >= resolved.min_samples
            and all(count >= resolved.min_samples_per_city for count in validation.counts_by_city.values()),
            f"samples={samples}; cities={dict(validation.counts_by_city)}",
        ),
        _gate("EXTRACTION_QUALITY", extraction >= resolved.min_extraction_exact_rate, f"extraction_exact_rate={extraction:.6f}"),
        _gate("CONFIDENCE_POLICY", low_confidence <= resolved.max_low_confidence_rate, f"low_confidence_rate={low_confidence:.6f}"),
        _gate("RESOLUTION_QUALITY", resolution >= resolved.min_resolution_rate, f"resolution_rate={resolution:.6f}"),
        _gate(
            "DISTANCE_QUALITY",
            within >= resolved.min_within_tolerance_rate
            and median is not None
            and p95 is not None
            and float(median) <= resolved.max_median_distance_m
            and float(p95) <= resolved.max_p95_distance_m,
            f"within_tolerance_rate={within:.6f}; median={median}; p95={p95}",
        ),
        _gate("KIND_QUALITY", kind >= resolved.min_kind_accuracy, f"kind_accuracy={kind:.6f}"),
        _gate(
            "CITY_QUALITY",
            bool(cities)
            and set(cities) == set(validation.cities)
            and all(
                float(_mapping(row, f"metrics.cities.{city}").get("resolution_rate", 0.0))
                >= resolved.min_city_resolution_rate
                and float(_mapping(row, f"metrics.cities.{city}").get("within_tolerance_rate", 0.0))
                >= resolved.min_city_within_tolerance_rate
        ...
        _gate(
            ...
            "qualification may use public Nominatim; production requires a dedicated endpoint",
        ),
    )
...
def extraction_exact_rate(
    ...
    expected = {item.message_key: item.expected_mention.casefold() for item in validation.cases}
    predicted = {
...
def low_confidence_rate(predictions: Sequence[MessageGeolocationResult]) -> float:
    ...
    if not selected:
        return 1.0
    return round(sum(not item.included_for_analysis for item in selected) / len(selected), 6)
...
def load_qualified_registry(path: Path) -> FrozenDict:
    with Path(path).open("r", encoding="utf-8") as stream:
    ...
    declared = _sha256(payload.get("registry_digest"), "registry.registry_digest")
    content = dict(payload)
    content.pop("registry_digest", None)
    if digest_json(content) != declared:
