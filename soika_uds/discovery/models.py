...
class GeoScope:
    ...
    def __post_init__(self) -> None:
        for name in ("raw_address", "city", "precision", "candidate_id", "label"):
            object.__setattr__(self, name, _required_text(getattr(self, name), name))
        for name in ("region", "district", "street", "house_number", "osm_type"):
        ...
        if self.osm_id is not None and (not isinstance(self.osm_id, int) or self.osm_id < 1):
            raise ValueError("osm_id must be a positive integer")
        aliases = tuple(
            dict.fromkeys(_required_text(item, "aliases[]") for item in self.aliases)
        )
...
class DiscoveryQuery:
    ...
    def __post_init__(self) -> None:
        ...
        object.__setattr__(self, "purpose", _required_text(self.purpose, "query.purpose"))
        if self.target_kind is not None and not isinstance(self.target_kind, SourceKind):
            object.__setattr__(self, "target_kind", SourceKind(self.target_kind))
...
class SearchHit:
    ...
    def __post_init__(self) -> None:
        object.__setattr__(self, "query", _required_text(self.query, "hit.query"))
        object.__setattr__(self, "title", _required_text(self.title, "hit.title"))
        ...
        object.__setattr__(self, "provider", _required_text(self.provider, "hit.provider"))
...
class SourceCandidate:
    ...
    def __post_init__(self) -> None:
        ...
        for name in ("title", "discovered_by", "query"):
            object.__setattr__(self, name, _required_text(getattr(self, name), name))
        evidence = tuple(
            dict.fromkeys(_required_text(item, "geo_evidence[]") for item in self.geo_evidence)
        )
...
class SourceOutcome:
    ...
    def __post_init__(self) -> None:
        ...
        if not isinstance(self.reason_code, SourceReasonCode):
            object.__setattr__(self, "reason_code", SourceReasonCode(self.reason_code))
        object.__setattr__(self, "reason", _required_text(self.reason, "reason"))
        urls = tuple(canonical_url(item) for item in self.attempted_urls)
        object.__setattr__(self, "attempted_urls", tuple(dict.fromkeys(urls)))
...
class DiscoveryPlan:
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            ...
            "provider": self.provider,
            "active_source_kinds": sorted(item.value for item in ACTIVE_SOURCE_KINDS),
            ...
            "stats": {
                ...
                "excluded_candidates": len(self.candidates) - len(self.active_candidates),
            },
        }
