...
Общий контракт идентификаторов источников синхронизирован с каталогом коннекторов: двухсимвольный идентификатор `vk` поддерживается `SourcePolicy` и `ParserRequest` и проверяется полным unit-набором.
