...
GeoJSON-линия не переобъявляется как метрическая без преобразования. Для расстояния координаты реально преобразуются через `pyproj.Transformer(..., always_xy=True)` в локальную UTM-зону; вне диапазона UTM используется `EPSG:3857` как явный fallback. В результат одновременно записываются исходная и метрическая CRS.
...
Вес каждого показателя и его contribution присутствуют в результате. Сумма весов обязана быть равна 1.0. Score ограничен диапазоном `[0, 1]`.
