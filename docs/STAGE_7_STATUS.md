...
Статическая нормализация запущена отдельным контролируемым commit-trigger перед обязательным CI. Финальный статус будет установлен после lock consistency и CPU/GPU Docker-проверки.
